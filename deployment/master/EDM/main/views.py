...
from utilities.lookup import lookup_service
...
def result():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            file_id = str(dbOps.saveFileToDB(mgInstance.mongo, current_user.username, filename_str, all_content))

            # save file to path
            output_dir = output_dir + "/" + filename_str + "/" + file_id
            ...
            # Add by Liuyin Huang
            container_op.upload_code(session['username'], filename_str, file_id)
...
def run_instance():
    if request.method == 'POST':
        user_name, domain_model_name, file_id = \
            session['username'], request.form['domainModelName'], request.form['fileId']
        ...
        # Add by Liuyin Huang
        container_op.start_container(user_name, domain_model_name, file_id)
...
def stop_instance():
    if request.method == 'POST':
        ...
        # Add by Liuyin Huang
        container_op.stop_container(user_name, domain_model_name, file_id)
...
def delete_instance():
    """
    look up ip
    call api to delete web & mongo containers
    :return:
    """
    if request.method == 'POST':
        user_name, domain_model_name, file_id = \
        ...
        # Add by Liuyin Huang
        container_op.delete_container(user_name, domain_model_name, file_id)

        dbOps.deleteInstanceFromDB(mgInstance.mongo, user_name, domain_model_name, file_id)

        file_dir = os.path.join(config.get('Output', 'output_path')) + "/" + user_name + "/" + domain_model_name + "/" + file_id
...
def detail_api():
    ...
    instance_id = request.args['fileId']
    ip_port = lookup_service(instance_id)
    server_url = ip_port + "/api"
    print(instance_id + " -> " + server_url)
...
def regenerate():
    ...
    # TODO: use the old same file_id instead of generating a new one, so output new code to the old folder?
    output_dir = os.path.join(config.get('Output', 'output_path')) + "/" + session['username']
    ...
    # TODO: why overwrite description_data?
    description_data = {
        "fileId": file_id,
        "domainModelName" : domain_model_name
    }

    # container_op.upload_code(session['username'], domain_model_name, file_id)
