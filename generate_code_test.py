...
from utilities.template_utils import generate_code
...
def generate_model(json_data, template_filename, dm_name):
    ...
    for model_name in json_data:
        for element in json_data.get(model_name).get("elements"):
            ...
            data["attributes"] = attribute_list
            ...
            data["methods"] = final_str

            content = generate_code(content, data)
...
def generate_adapter(adapter_filename, server_ip, port, dm_name):
    ...
    data = {"server_ip": server_ip,
            "port": port}
    content = generate_code(adapter_template_file, data)
...
def generate_server(server_filename, server_ip, port, dm_name):
    ...
    data = {"server_ip": server_ip,
            "port": port,
            "dbname": dm_name}
    content = generate_code(server_template_file, data)
