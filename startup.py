...
from config import config, runtime_config

from main.views import main_bp
from main.login_manager import login_manager
...
# mgInstance.getInstance().initializeMongo(app)
...
def create_app(config_name = 'dev'):
    app = Flask(__name__)
    app.config.from_object(runtime_config[config_name])
    mgInstance.getInstance().initializeMongo(app)
...
# from authen import requesttoken, verifykeyapi, verifykey
...
# def verifykey(username, usertoken):
#     authentcol = mgInstance.mongo.db.authentication
#     findresult = authentcol.find_one({"username": username})
#     if findresult == None:
#         return False
...
#     # delta = dt.datetime.now(pytz.utc) - validtill
#     if delta >= dt.timedelta():
#         User.refreshToken(username, mgInstance.mongo)
#         findresult = authentcol.find_one({"username": username})
#     if findresult["key"] == usertoken:
#         return True
